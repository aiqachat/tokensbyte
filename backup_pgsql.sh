...
# Docker容器名称
DOCKER_CONTAINER_NAME="${PROJECT_NAME}-postgres"
# 数据库用户名
DB_USER="tokensapi"
# 数据库密码（留空则使用容器默认认证方式）
DB_PASSWORD=""
...
# 备份文件名称（带时间戳和项目名）
BACKUP_FILENAME="${PROJECT_NAME}_pgsql_backup_$(date +%Y%m%d_%H%M%S).sql.gz"
# 备份文件完整路径
...
clean_expired_backups() {
    if [ "${RETENTION_DAYS}" -gt 0 ]; then
        ...
        find "${BACKUP_DIR}" -name "${PROJECT_NAME}_pgsql_backup_*.sql.gz" -type f -mtime +"${RETENTION_DAYS}" -delete
        echo "过期备份清理完成"
    fi
}
