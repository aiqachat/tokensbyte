...
if [[ "$(uname -s)" == "Darwin" ]]; then
    if [ -z "$DOCKER_DEFAULT_PLATFORM" ]; then
        echo "🍎 检测到 Mac 环境，请选择目标服务器的架构："
        echo "   [1] linux/amd64 (默认，适用于绝大多数 x86_64 云服务器)"
        echo "   [2] linux/arm64 (适用于 ARM 架构服务器，如这台内网测试机)"
        read -p "请输入 [1/2] (默认 1): " ARCH_CHOICE
        if [ "$ARCH_CHOICE" = "2" ]; then
            export DOCKER_DEFAULT_PLATFORM=linux/arm64
            echo "✅ 已设置构建架构为 linux/arm64"
        else
            export DOCKER_DEFAULT_PLATFORM=linux/amd64
            echo "✅ 已设置构建架构为 linux/amd64"
        fi
fi
