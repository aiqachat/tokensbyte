...
| SQLx | 最新 | 数据库 ORM，仅支持 PostgreSQL（`postgres` feature） |
...
| 技术 | 版本 | 用途 |
|------|------|------|
| React | 19.x | 现代化前端渲染框架，提升性能与体验 |
| TypeScript | 6.x | 类型安全的 JavaScript 超集，确保开发规范 |
| Ant Design | 6.x | 核心 UI 组件库，用于复杂后台数据表单和管理布局 |
| Shadcn UI | 最新 | 无头组件库结合 HSL 变量，用于自定义高度定制化交互组件 |
| Tailwind CSS | v4.x | 基于 CSS-first 架构的极速样式引擎，配合 `@tailwindcss/vite` |
| Zustand | 5.x | 轻量且高弹性的状态管理库 |
| React Router | 6.x | 单页应用 (SPA) 路由管理 |
| i18next | 最新 | 国际化多语言支持 |
| Vite | 8.x | 前端构建工具，提供秒级热重载 (HMR) 体验 |
...
| PostgreSQL | 16+ / 18.4 | **唯一数据库**，存储全部业务数据（强制 INT8/BIGINT 整数规范，内置支持 18.4） |
...
>
> 后端镜像默认在容器内编译（Win/Mac/Linux 都安全）。若已有 **Linux** 预编译二进制想加速打包：把文件放到仓库根目录 `tokensbyte-server-bin`，再执行 `USE_PREBUILT=1 docker compose build`（架构不符会构建失败，避免再出现线上无法识别）。
>
...
本地模式会自动复用已在运行的 Docker Postgres（默认 `localhost:5432`），并在 `3000` / `5173` 被占用时自动顺延空闲端口（如 `3001` / `5174`），不会杀掉另一套代码的进程。也可手动指定：

```bash
# 第二套仓库示例
BACKEND_PORT=3001 FRONTEND_PORT=5174 ./dev.sh
```

```powershell
# Windows 第二套仓库示例
$env:BACKEND_PORT=3001; $env:FRONTEND_PORT=5174; .\dev.ps1
```

**Windows 中文路径：** `dev.ps1` 会自动切换控制台 UTF-8、将 `CARGO_TARGET_DIR` 重定向到 `%LOCALAPPDATA%\tokensbyte-dev\target\...` 纯英文目录，并用 `Start-Process` 代替易在中文路径下失败的 `Start-Job`。若仍编译失败，建议安装 Visual Studio Build Tools (MSVC)，或将仓库放到纯英文路径。
...
│
├── docker-compose.yml      # Docker Compose 配置（内置 PostgreSQL 18.4，可切换外部 PostgreSQL）
...
- **详情清理**：`storage_settings.log_retention_days`（默认 30）清空超期行的请求/响应大字段，行仍留在 `logs`。
- **行归档**：`storage_settings.log_row_retention_days`（默认 0=关闭）把超期行迁入 `logs_archive` 并从热表删除；实际阈值 = 配置天数 + 2。
- **TIMESTAMPTZ 自检 SQL / 月分区 Phase 2**：见 `UPDATE.md` 对应章节。
...
A: 使用 PostgreSQL 原生工具 `pg_dump` 和 `pg_restore` 进行数据备份 and 迁移。如需升级数据库版本，系统会在启动时自动执行增量迁移脚本，无需手动干预。项目根目录提供了 `backup_pgsql.sh`、`upgrade-postgres.sh` (15->16 升级脚本) 和 `upgrade-postgres-18.sh` (16->18.4 升级脚本) 辅助运维。
