...
> [!IMPORTANT]
> **数据库强制要求：本项目唯一支持的数据库为 PostgreSQL 16+。**
...
| Ant Design | 5.x | UI 组件库 |Shadcn UI｜
| Zustand | 最新 | 轻量级状态管理 |
...
| PostgreSQL | 16+ | **唯一数据库**，存储全部业务数据（强制 INT8/BIGINT 整数规范） |
...
无需任何配置，直接运行即可启动全部服务（内置 PostgreSQL 16 + 默认配置）：
...
│
├── docker-compose.yml      # Docker Compose 配置（内置 PostgreSQL 16，可切换外部 PostgreSQL）
...
A: **本项目仅支持 PostgreSQL（16+），不支持 MySQL、SQLite 或其他数据库。** 后端 `sqlx` 编译时仅启用了 `postgres` feature，所有 SQL 语句、迁移脚本和数据类型均严格遵循 PostgreSQL 规范。

- **内置 PostgreSQL**：`docker-compose.yml` 内置了 PostgreSQL 16-alpine 服务，支持 `docker compose up -d` 一键启动
...
A: 使用 PostgreSQL 原生工具 `pg_dump` 和 `pg_restore` 进行数据备份和迁移。如需升级数据库版本，系统会在启动时自动执行增量迁移脚本，无需手动干预。项目根目录提供了 `backup_pgsql.sh` 和 `upgrade-postgres.sh` 脚本辅助运维。
