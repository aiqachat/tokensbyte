...
与 New-api 的对比本系统主要是重点兼容中国各种的大模型的转发和计费，完全自主研发给现在做 api 业务的人群多一套完整开源系统的选择。
...
```bash
git clone https://github.com/aiqachat/tokensbyte-ws.git
cd tokensbyte
...
- **提交信息**：使用语义化提交规范，格式为 `type(scope): description`（例如 `feat(auth): 增加三方登录组件`）
...
- 项目问题：[GitHub Issues](https://github.com/aiqachat/tokensbyte-ws/issues)
