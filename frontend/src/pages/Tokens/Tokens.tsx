      ...
      render: (record: ApiToken) => (
        <Space direction="vertical" size={2}>
          <Text type="secondary" style={{ fontSize: 12 }}>
            Used: {record.quota_used.toFixed(4)}
          </Text>
          <Text style={{ fontSize: 12 }}>
            Limit: {record.quota_limit < 0 ? 'Unlimited' : record.quota_limit}
          </Text>
        </Space>
      ),
    },
    ...
    },
    {
      ...
      render: (_: unknown, record: ApiToken) => (
        <Space>
          ...
          <Popconfirm title="Delete token?" onConfirm={() => handleDelete(record.id)}>
  ...
  return (
    <Card bordered={false}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
        <Title level={2} style={{ margin: 0 }}>API Tokens</Title>
        <Space>
          <Button icon={<SyncOutlined />} onClick={fetchTokens}>Refresh</Button>
          <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>Create Token</Button>
        </Space>
      </div>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSave}>
          <Form.Item name="name" label="Token Display Name" rules={[{ required: true }]} initialValue="default">
            <Input placeholder="e.g. Project A" />
          </Form.Item>

          <Form.Item name="quota_limit" label="Quota Limit (-1 for unlimited)" initialValue={-1}>
            <InputNumber style={{ width: '100%' }} />
          </Form.Item>

          <Form.Item name="allowed_models" label="Limit to Models (one per line, empty for all)">
