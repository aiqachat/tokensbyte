    ...
    <div>
        ...
        <div style={{
          ...
          <div ref={logPanelRef} style={{
            ...
            {syncLogs.length === 0 && syncing && (
              <Text style={{ color: 'rgba(255,255,255,0.25)' }}>等待服务器响应...</Text>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <Input prefix={<SearchOutlined style={{ color: 'rgba(255,255,255,0.25)' }} />} placeholder="搜索图标名称..."
      ...
      <Text style={{ color: 'rgba(255,255,255,0.45)', fontSize: 13, display: 'block', marginBottom: 16 }}>
        共 <span style={{ color: '#fff', fontWeight: 500 }}>{total}</span> 个图标
      </Text>
      ...
      {loading && icons.length === 0 ? (
        ...
        <Empty description={<Text style={{ color: 'rgba(255,255,255,0.35)' }}>暂无图标，点击「在线更新」从 Lobe Icons 同步</Text>} style={{ padding: 60 }} />
      ) : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: 12 }}>
            {icons.map(icon => (
              <div key={icon.id} style={{
                background: '#141414', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8,
              ...
              >
                ...
                <div style={{ width: 48, height: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8, background: 'rgba(255,255,255,0.04)', borderRadius: 8, overflow: 'hidden' }}>
                ...
                <Text style={{ color: 'rgba(255,255,255,0.65)', fontSize: 11, textAlign: 'center', lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%' }} title={icon.title || icon.name}>{icon.title || icon.name}</Text>
                <div className="icon-actions" style={{ position: 'absolute', bottom: 4, right: 4, display: 'flex', gap: 2, opacity: 0, transition: 'opacity 0.15s' }} onClick={e => e.stopPropagation()}>
                  <Tooltip title="编辑"><Button type="text" size="small" icon={<EditOutlined />} style={{ color: 'rgba(255,255,255,0.45)', fontSize: 12 }} onClick={() => handleOpenEdit(icon)} /></Tooltip>
      ...
      <Modal title={editingIcon ? '编辑图标' : '添加自定义图标'} open={addModalVisible} onOk={handleSave} onCancel={() => setAddModalVisible(false)} confirmLoading={saving} okText={editingIcon ? '保存修改' : '添加图标'} width={520} destroyOnClose>
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item label="SVG 内容" required={!editingIcon}>
            <Dragger accept=".svg" showUploadList={false} beforeUpload={handleSvgUpload as any} style={{ background: '#1a1a1a', borderColor: 'rgba(255,255,255,0.12)' }}>
              <p style={{ color: 'rgba(255,255,255,0.35)' }}><InboxOutlined style={{ fontSize: 32 }} /></p>
              <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 13 }}>点击或拖拽 SVG 文件到此处</p>
            </Dragger>
            <div style={{ marginTop: 8 }}>
              <Text style={{ color: 'rgba(255,255,255,0.35)', fontSize: 12 }}>或直接粘贴 SVG 代码：</Text>
              <TextArea rows={4} placeholder="<svg ...>...</svg>" value={svgPreview} onChange={e => setSvgPreview(e.target.value)} style={{ marginTop: 4, background: '#1a1a1a', borderColor: 'rgba(255,255,255,0.12)', fontFamily: 'monospace', fontSize: 12 }} />
            </div>
            ...
            <div style={{ textAlign: 'left', padding: '0 12px' }}>
              {[['标识名', previewIcon.name], ['显示名称', previewIcon.title || '-'], ['分类', previewIcon.category], ['文件路径', previewIcon.file_path], ['更新时间', previewIcon.updated_at]].map(([label, val]) => (
                <div key={label as string} style={{ marginBottom: 8 }}>
                  <Text style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>{label}</Text>
              ...
              <div style={{ marginBottom: 8 }}><Text style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>来源</Text><Tag color={previewIcon.source === 'custom' ? 'green' : 'blue'} style={{ marginLeft: 8 }}>{previewIcon.source === 'custom' ? '手动添加' : 'Lobe Icons'}</Tag></div>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal title="同步历史日志" open={logModalVisible} onCancel={() => setLogModalVisible(false)} footer={null} width={600}>
        {logsLoading ? <div style={{ textAlign: 'center', padding: 40 }}><Spin /></div>
         ...
         : <div style={{ maxHeight: 400, overflow: 'auto' }}>
            {syncLogList.map(log => (
              <div key={log.id} style={{ background: '#141414', borderRadius: 6, border: '1px solid rgba(255,255,255,0.06)', padding: '12px 16px', marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  ...
                  <Text style={{ color: 'rgba(255,255,255,0.35)', fontSize: 12 }}>{log.created_at}</Text>
                </div>
                <Text style={{ color: 'rgba(255,255,255,0.65)', fontSize: 13 }}>同步 {log.total_synced} 个，库中共 {log.total_new} 个</Text>
