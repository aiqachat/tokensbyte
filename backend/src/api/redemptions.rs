...
mod expiry_tests {
    ...
    #[test]
    fn none_or_empty_never_expired() {
        assert!(!is_expired(None, "Asia/Shanghai"));
        assert!(!is_expired(Some(""), "Asia/Shanghai"));
        assert!(!is_expired(Some("   "), "Asia/Shanghai"));
    }

    #[test]
    fn past_and_future_date_only() {
        assert!(is_expired(Some("2020-01-01"), "Asia/Shanghai"));
        assert!(!is_expired(Some("2099-12-31"), "Asia/Shanghai"));
    }

    #[test]
    fn naive_datetime_uses_site_timezone_not_utc() {
        // 按上海墙钟：2020 已过期；2099 未过期
        assert!(is_expired(Some("2020-06-01 12:00:00"), "Asia/Shanghai"));
        assert!(!is_expired(Some("2099-06-01 12:00:00"), "Asia/Shanghai"));
    }

    #[test]
    fn normalize_keeps_calendar_date() {
        assert_eq!(
            normalize_expiry_date("2026-07-17", "Asia/Shanghai").unwrap(),
            "2026-07-17"
        );
        assert_eq!(
            normalize_expiry_date("2026-07-17 23:59:59", "Asia/Shanghai").unwrap(),
            "2026-07-17"
        );
    }
}
