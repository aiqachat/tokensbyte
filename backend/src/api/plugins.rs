...
mod api_access_tests {
    ...
    #[test]
    fn level_mode_uses_group_then_default() {
        let c = cfg(&[("api_enabled_vip", "false"), ("api_enabled", "true")]);
        assert!(!is_asset_api_enabled(&c, "u1", "vip", Some(2)));
        assert!(is_asset_api_enabled(&c, "u1", "default", Some(1)));
    }

    #[test]
    fn level_mode_falls_back_to_level_id_and_defaults_off() {
        let c = cfg(&[("api_enabled_3", "false")]);
        assert!(!is_asset_api_enabled(&c, "u1", "unknown", Some(3)));
        // 未配置且无全局默认时，默认关闭
        assert!(!is_asset_api_enabled(&c, "u1", "unknown", Some(9)));
        let c2 = cfg(&[("api_enabled", "true")]);
        assert!(is_asset_api_enabled(&c2, "u1", "unknown", Some(9)));
    }

    #[test]
    fn user_include_and_exclude() {
        let c = cfg(&[
            ("api_access_mode", "user"),
            ("api_user_mode", "include"),
            ("api_user_ids", r#"["a","b"]"#),
        ]);
        assert!(is_asset_api_enabled(&c, "a", "vip", None));
        assert!(!is_asset_api_enabled(&c, "c", "vip", None));

        let c2 = cfg(&[
            ("api_access_mode", "user"),
            ("api_user_mode", "exclude"),
            ("api_user_ids", r#"["a"]"#),
        ]);
        assert!(!is_asset_api_enabled(&c2, "a", "vip", None));
        assert!(is_asset_api_enabled(&c2, "c", "vip", None));
    }

    #[test]
    fn user_include_empty_denies_all() {
        let c = cfg(&[
            ("api_access_mode", "user"),
            ("api_user_mode", "include"),
            ("api_user_ids", "[]"),
        ]);
        assert!(!is_asset_api_enabled(&c, "anyone", "vip", None));
    }

    #[test]
    fn level_group_key_takes_precedence_over_id() {
        let c = cfg(&[("api_enabled_vip", "true"), ("api_enabled_2", "false")]);
        assert!(is_asset_api_enabled(&c, "u1", "vip", Some(2)));
    }
}
