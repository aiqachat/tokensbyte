...
pub async fn delete_provider(
    ...
    let _is_sys: i32 = sqlx::query_scalar(&state.db.format_query("SELECT is_system FROM model_providers WHERE id = ?"))
        .bind(id)
        .fetch_one(&state.db.pool)
        .await
        .unwrap_or(0);
...
pub async fn list_api_providers(
    ...
    let providers = sqlx::query_as(&state.db.format_query("SELECT * FROM model_api_providers ORDER BY sort_order ASC, id ASC"))
        .fetch_all(&state.db.pool)
        .await?;
    Ok(Json(providers))
}
...
pub async fn create_api_provider(
    ...
    let exists: Option<i64> = sqlx::query_scalar(&state.db.format_query("SELECT id FROM model_api_providers WHERE name = ?"))
        .bind(&req.name).fetch_optional(&state.db.pool).await?;
    if exists.is_some() {
        return Err(crate::error::AppError::Conflict("API服务商名称已存在".to_string()));
    }
...
pub async fn update_api_provider(
    ...
    let exists: Option<i64> = sqlx::query_scalar(&state.db.format_query("SELECT id FROM model_api_providers WHERE name = ? AND id != ?"))
        .bind(&req.name).bind(id).fetch_optional(&state.db.pool).await?;
    if exists.is_some() {
        return Err(crate::error::AppError::Conflict("API服务商名称与其他记录重复".to_string()));
    }
...
pub async fn delete_api_provider(
    ...
    sqlx::query(&state.db.format_query("UPDATE models SET api_provider_id = NULL WHERE api_provider_id = ?"))
        .bind(id).execute(&state.db.pool).await?;
    sqlx::query(&state.db.format_query("DELETE FROM model_api_providers WHERE id = ?"))
        .bind(id).execute(&state.db.pool).await?;
    crate::api::plugins::notify_marketplace_data_changed(&state).await;
    Ok(Json(serde_json::json!({ "success": true })))
}
...
pub async fn delete_type(
    ...
    let _is_sys: i32 = sqlx::query_scalar(&state.db.format_query("SELECT is_system FROM model_types WHERE id = ?"))
        .bind(id)
        .fetch_one(&state.db.pool)
        .await
        .unwrap_or(0);
...
pub async fn get_classifications_stats(
    ...
    p_sql.push_str(" WHERE p.is_active = 1 GROUP BY p.id, p.name, p.name_en, p.logo, p.is_system ORDER BY p.sort_order ASC, p.id ASC");
    ...
    let mut ap_sql = r#"SELECT p.id, p.name, p.name_en, p.logo, p.is_system, COUNT(m.id) as count 
           ...
           LEFT JOIN models m ON p.id = m.api_provider_id"#.to_string();
    ...
    ap_sql.push_str(" WHERE p.is_active = 1 GROUP BY p.id, p.name, p.name_en, p.logo, p.is_system ORDER BY p.sort_order ASC, p.id ASC");
    ...
    t_sql.push_str(" WHERE t.is_active = 1 GROUP BY t.id, t.name, t.name_en, t.logo, t.is_system ORDER BY t.sort_order ASC, t.id ASC");
    ...
    Ok(Json(ClassificationsResponse { providers, api_providers, types }))
}
