...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    let mut admin_routes: Router<Arc<AppState>> = Router::new()
        ...
        .route("/model-api-providers", get(model_classifications::list_api_providers).post(model_classifications::create_api_provider))
        .route("/model-api-providers/{id}", put(model_classifications::update_api_provider).delete(model_classifications::delete_api_provider))
    ...
    admin_routes = admin_routes
        ...
        .route("/settings/storage/test", post(settings::test_storage_connection))
    ...
    #[cfg(plugin_finance)]
    {
        admin_routes = admin_routes
            ...
            .route("/finance/recharges/stats_batch", post(finance::get_wallet_stats_batch))
    ...
    management_routes = management_routes
        ...
        .route("/task_logs/{log_id}/cancel", post(task_logs::cancel_task_log))
    ...
    #[cfg(feature = "plugin_volcengine_pool")]
    let management_routes = management_routes.nest("/plugins/volcengine_pool", volcengine_pool::router());
    #[cfg(feature = "plugin_gptimage_pool")]
    let management_routes = management_routes.nest("/plugins/gptimage_pool", gptimage_pool::router());
    ...
    let management_routes = management_routes.nest("/plugins/router-flow", router_flow::router());
    #[cfg(feature = "plugin_site_portal")]
    let management_routes = management_routes.nest("/plugins/site-portal", site_portal::admin_router());
    #[cfg(feature = "plugin_happyhorse")]
    let management_routes = management_routes.nest("/plugins/happyhorse_router", happyhorse_router::router());
    let management_routes = management_routes
    ...
    let relay_routes: Router<Arc<AppState>> = Router::new()
        .route("/chat/completions", post(crate::relay::chat::chat_completions))
        ...
        .route("/images/edits", post(crate::relay::image::image_generations))
        .route("/video/generations", post(crate::relay::video::video_generations))
        .route("/video/generations/{task_id}", get(crate::relay::task::task_status))
        ...
        .route("/videos/text2video/{task_id}", get(crate::relay::task::task_status))
        .route("/videos/image2video/{task_id}", get(crate::relay::task::task_status))
        .route("/videos/multi-image2video/{task_id}", get(crate::relay::task::task_status))
        .route("/videos/omni-video/{task_id}", get(crate::relay::task::task_status))
        // 可灵 AI 原生图片路径
    ...
    let dashscope_native_routes: Router<Arc<AppState>> = Router::new()
        .route("/api/v1/services/aigc/video-generation/video-synthesis", post(crate::relay::video::video_generations))
        .route("/api/v1/services/aigc/multimodal-generation/generation", post(crate::relay::image::image_generations))
        .route("/api/v1/tasks/{task_id}", get(crate::relay::task::task_status))
        // 阿里百炼 DashScope 文本向量（OpenAI 兼容模式）
        .route("/compatible-mode/v1/embeddings", post(crate::relay::generic::generic_relay))
        // 阿里百炼 DashScope 排序（兼容模式，qwen3-rerank）
        .route("/compatible-api/v1/reranks", post(crate::relay::generic::generic_relay))
        // 阿里百炼 DashScope 排序（原生模式，gte-rerank-v2）
        .route("/api/v1/services/rerank/text-rerank/text-rerank", post(crate::relay::generic::generic_relay))
        .layer(axum_middleware::from_fn_with_state(state.clone(), api_key_middleware))
        .with_state(state.clone());
    ...
    let volcengine_native_routes: Router<Arc<AppState>> = Router::new()
        .route("/api/v3/chat/completions", post(crate::relay::chat::chat_completions))
        .route("/api/v3/responses", post(crate::relay::chat::responses_create))
        .route("/api/v3/contents/generations/tasks", post(crate::relay::video::video_generations).get(crate::relay::native::volcengine_task_list))
        .route("/api/v3/contents/generations/tasks/{task_id}", get(crate::relay::task::task_status).delete(crate::relay::native::volcengine_task_cancel))
        .route("/api/v3/images/generations", post(crate::relay::image::image_generations))
        // 火山方舟原生语音合成路由（SSE + HTTP Chunked 两种传输协议）
        .route("/api/v3/tts/unidirectional/sse", post(crate::relay::audio::audio_speech))
        .route("/api/v3/tts/unidirectional", post(crate::relay::audio::audio_speech))
        // 模型列表
        .route("/api/v3/models", get(crate::relay::model_list::list_models))
        .route("/api", post(crate::relay::native::ark_asset_proxy))
        .layer(axum_middleware::from_fn_with_state(state.clone(), api_key_middleware))
        .layer(axum_middleware::from_fn(crate::relay::native::normalize_volcengine_auth))
        .with_state(state.clone());
...
#[cfg(feature = "plugin_volcengine_pool")]
pub mod volcengine_pool;    // Plugin: 火山引擎卡池
#[cfg(feature = "plugin_gptimage_pool")]
pub mod gptimage_pool;      // Plugin: GPT Image 卡池
#[cfg(feature = "plugin_site_icons")]
pub mod site_icons;         // Plugin: 站点图标管理
#[cfg(feature = "plugin_router_flow")]
pub mod router_flow;        // Plugin: 智能路由流
#[cfg(feature = "plugin_site_portal")]
pub mod site_portal;        // Plugin: 站点门户
#[cfg(feature = "plugin_happyhorse")]
pub mod happyhorse_router;  // Plugin: 快乐小马智能路由
