...
async fn list_pools(
    ...
    for pool in &pools {
        ...
        let accounts: Vec<VolcenginePoolAccountMapping> = sqlx::query_as(
...
async fn list_accounts(
    ...
    let accounts: Vec<VolcenginePoolAccount> = sqlx::query_as(
        &state.db.format_query("SELECT * FROM volcengine_pool_accounts ORDER BY id ASC"),
    )
    .fetch_all(&state.db.pool)
    .await?;
...
async fn create_account(
    ...
    let account: VolcenginePoolAccount = sqlx::query_as(&state.db.format_query(
        "INSERT INTO volcengine_pool_accounts (name, base_url, api_key, account_id, access_key, secret_key, models) \
         VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING *",
    ))
    .bind(&req.name)
    .bind(req.base_url.as_deref().unwrap_or("https://ark.cn-beijing.volces.com/api/v3"))
    .bind(&req.api_key)
    .bind(req.account_id.as_deref().unwrap_or(""))
    .bind(req.access_key.as_deref().unwrap_or(""))
    .bind(req.secret_key.as_deref().unwrap_or(""))
...
async fn update_account(
    ...
    if let Some(aid) = req.account_id { account.account_id = aid; }
    if let Some(ak) = req.access_key { account.access_key = ak; }
    if let Some(sk) = req.secret_key { account.secret_key = sk; }
    if let Some(models) = req.models { account.models = models; }
    if let Some(status) = req.status { account.status = status; }

    sqlx::query(&state.db.format_query(
        "UPDATE volcengine_pool_accounts SET name = ?, base_url = ?, api_key = ?, account_id = ?, access_key = ?, secret_key = ?, models = ?, status = ?, \
