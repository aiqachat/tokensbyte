...
pub async fn create_user(
    ...
    crate::api::auth::validate_username(&request.username)?;
...
pub async fn update_user(
    ...
    let old_level_name: String = sqlx::query_scalar(&state.db.format_query(
        "SELECT COALESCE(name, ?) FROM user_levels WHERE group_key = ?"
    ))
    .bind(&old_user_group)
    .bind(&old_user_group)
    .fetch_optional(&state.db.pool)
    ...
    if let Some(username) = request.username {
        crate::api::auth::validate_username(&username)?;
    ...
    if let Some(ref md) = request.model_discounts {
        user.model_discounts = if md.is_empty() { None } else { Some(md.clone()) };
    }
    if let Some(timezone) = request.timezone { user.timezone = Some(timezone); }
    if let Some(credit_limit) = request.credit_limit { user.credit_limit = credit_limit; }
    if let Some(pay_enabled) = request.pay_enabled { user.pay_enabled = pay_enabled; }
    ...
    sqlx::query(
        &state.db.format_query(r#"UPDATE users SET username = ?, email = ?, password_hash = ?, 
           ...
           role = ?, balance = ?, gift_balance = ?, gift_used_quota = ?, user_group = ?, is_active = ?, admin_remark = ?, referred_by = ?, referral_history = ?, model_discounts = ?, timezone = ?, credit_limit = ?, pay_enabled = ?, updated_at = CURRENT_TIMESTAMP
           WHERE id = ?"#)
    )
    .bind(&user.username)
    .bind(&user.email)
    ...
    if (user.balance - old_balance).abs() > 1e-9 {
        ...
        let remark = if diff > 0.0 { "管理员调增系统余额" } else { "管理员调减系统余额" };
        sqlx::query(
            &state.db.format_query("INSERT INTO recharge_records (user_id, amount, recharge_type, remark, operator, wallet_type) VALUES (?, ?, 'manual', ?, ?, 'system')")
        )
        .bind(&id)
    ...
    if (user.gift_balance - old_gift_balance).abs() > 1e-9 {
        ...
        let remark = if diff > 0.0 { "管理员调增赠送余额" } else { "管理员调减赠送余额" };
        sqlx::query(
            &state.db.format_query("INSERT INTO recharge_records (user_id, amount, recharge_type, remark, operator, wallet_type) VALUES (?, ?, 'manual', ?, ?, 'gift')")
        )
        .bind(&id)
    ...
    if (user.credit_limit - old_credit_limit).abs() > 1e-9 {
        ...
        let remark = if diff > 0.0 { "管理员调增信控额度" } else { "管理员调减信控额度" };
        sqlx::query(
    ...
    if user.user_group != old_user_group {
        let new_level_name: String = sqlx::query_scalar(
            &state.db.format_query("SELECT COALESCE(name, ?) FROM user_levels WHERE group_key = ?")
        )
        .bind(&user.user_group)
        .bind(&user.user_group)
...
pub async fn delete_user(
    ...
    let assets: Vec<(String, String)> = sqlx::query_as::<_, (String, String)>(
        &state.db.format_query("SELECT file_url, plugin_ns FROM plugin_assets WHERE user_id = ?")
    )
    .bind(&id)
    .fetch_all(&state.db.pool)
    .await
    ...
    if !assets.is_empty() || !pg_assets.is_empty() {
        ...
        if !assets.is_empty() {
            ...
            for ns in assets.iter().map(|(_, ns)| ns).collect::<std::collections::HashSet<_>>() {
                    ...
                    if let Some(object_key) = tos_config.extract_object_key(&file_url) {
                        let task = tokio::spawn(async move {
                            match crate::services::tos::delete_file(&tos_config, &object_key).await {
                                Ok(()) => {
                                    tracing::info!("同步清理用户数据: plugin_assets TOS 文件删除成功: {}", object_key);
                                }
                                Err(e) => {
                                    tracing::warn!("同步清理用户数据: plugin_assets TOS 文件删除失败: {} - {}", object_key, e);
                                }
                            }
                        });
        ...
        if !pg_assets.is_empty() {
            if let Some(tos_config) = crate::api::plugins::get_tos_config(&state, "playground").await {
                for tos_key in pg_assets {
                    let tos_config = tos_config.clone();
                    let task = tokio::spawn(async move {
                        match crate::services::tos::delete_file(&tos_config, &tos_key).await {
                            Ok(()) => {
                                tracing::info!("同步清理用户数据: playground_assets TOS 文件删除成功: {}", tos_key);
                            }
                            Err(e) => {
                                tracing::warn!("同步清理用户数据: playground_assets TOS 文件删除失败: {} - {}", tos_key, e);
                            }
                        }
                    });
                    delete_tasks.push(task);
...
pub async fn recharge_user(
    ...
    if is_credit {
        // 信控额度操作
        let new_credit = user.credit_limit + request.amount;
        if new_credit < 0.0 {
            return Err(AppError::BadRequest("信控额度不能为负数".to_string()));
        }
        sqlx::query(&state.db.format_query("UPDATE users SET credit_limit = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"))
            .bind(new_credit)
            .bind(&id)
            .execute(&mut *tx)
            .await?;
    ...
    let wallet_type = if is_credit { "credit" } else if is_gift { "gift" } else { "system" };
