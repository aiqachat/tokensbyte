...
pub async fn update_settings(
    ...
    if let Some(ref v_json) = request.database {
        ...
        // 4. 初始化默认管理员账号（若新库没有 admin 用户）
        let admin_exists: Result<i64, _> =
            sqlx::query_scalar("SELECT COUNT(*) FROM users WHERE role = 'admin'")
                .fetch_one(&pool)
                .await;
        if let Ok(count) = admin_exists {
            if count == 0 {
                let password_hash = crate::auth::hash_password(&state.config.admin_password)?;
                ...
                let uid = loop {
                    ...
                    let c: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM users WHERE uid = $1")
                        .bind(&uid)
                        .fetch_one(&pool)
                        .await
                        .unwrap_or(0);
                ...
                let _ = sqlx::query(
                    r#"INSERT INTO users (id, uid, username, email, password_hash, role, balance, user_group, is_active, created_at, updated_at)
                       VALUES ($1, $2, $3, $4, $5, 'admin', 100.0, 'default', 1, $6, $7)"#
                )
                .bind(&id)
                .bind(&uid)
                .bind(&state.config.admin_username)
                .bind(format!("{}@tokensbyte.local", &state.config.admin_username))
                .bind(&password_hash)
                .bind(&now)
                .bind(&now)
                .execute(&pool)
