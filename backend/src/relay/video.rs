...
pub async fn video_generations(
    ...
    let (channel, resolved_model, resp, resolved, upstream_body, asset_convert_log) = loop {
        current_try += 1;
        ...
        tracing::info!("[Video] model={}, target_type={}, url={} (Attempt {}/{})", model, resolved.target_type, url, current_try, max_retries);
        ...
        match builder.json(&upstream_body).send().await {
            Ok(resp) => {
                if resp.status().is_success() {
                    break (channel, resolved_model, resp, resolved, upstream_body, asset_convert_log);
                } else {
            ...
            Err(e) => {
                ...
                proxy::record_and_bill(&state, &token, channel.id, model, 0, 0, 0, 0.0, 500,
                    ...
                    Some(request_content_str.clone()), Some(err_msg.clone()), Some(upstream_body.to_string()), asset_convert_log.clone()).await;
