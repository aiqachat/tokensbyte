...
impl AlipayClient {
    ...
    pub fn generate_page_pay_url(&self, out_trade_no: &str, amount: f64, subject: &str, notify_url: &str, return_url: &str) -> Result<String> {
        ...
        Ok(format!("https://openapi.alipay.com/gateway.do?{}", query_str))
    }
    ...
    pub fn verify_signature(&self, params: &BTreeMap<String, String>, signature: &str) -> Result<bool> {
        let sign_str = params.iter()
        ...
        if !pub_str.starts_with("-----BEGIN PUBLIC KEY-----") {
            pub_str = format!("-----BEGIN PUBLIC KEY-----\n{}\n-----END PUBLIC KEY-----", pub_str.chars().collect::<Vec<char>>().chunks(64).map(|c| c.into_iter().collect::<String>()).collect::<Vec<String>>().join("\n"));
        }
